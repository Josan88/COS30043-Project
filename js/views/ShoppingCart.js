/**
 * ShoppingCart Component v2.0.0 - Enhanced Edition
 * 
 * FEATURES:
 * - ✅ Cart Management: Add, remove, update quantities with real-time calculations
 * - ✅ Service Options: Dine-in (table selection), pickup, delivery with validation
 * - ✅ Promotions: Promo codes, bulk discounts, free delivery thresholds
 * - ✅ Payment Processing: Card, online banking, cash with comprehensive validation
 ...
 * - Analytics tracking with session management
...
const ShoppingCart = {
  ...
  data() {
    return {
      ...
      bulkDiscountRate: window.APP_CONSTANTS?.PRICING?.BULK_DISCOUNT_RATE || 0.05, // 5% bulk discount
      bulkDiscountThreshold: window.APP_CONSTANTS?.PRICING?.BULK_DISCOUNT_THRESHOLD || 3, // Apply to items with quantity >= 3
      recommendedProducts: [],
      cartTotals: {
        subtotal: 0,
        serviceCharge: 0,
      ...
      config: {
        pricing: {
          serviceChargeRate: window.APP_CONSTANTS?.PRICING?.SERVICE_CHARGE_RATE || 0.05,
          taxRate: window.APP_CONSTANTS?.PRICING?.TAX_RATE || 0.06,
          freeDeliveryThreshold: window.APP_CONSTANTS?.PRICING?.FREE_DELIVERY_THRESHOLD || 50,
          deliveryFee: window.APP_CONSTANTS?.PRICING?.DELIVERY_FEE || 5,
          minOrderAmount: window.APP_CONSTANTS?.PRICING?.MIN_ORDER_AMOUNT || 5
        },
        validation: {
          tableNumberPattern: window.APP_CONSTANTS?.VALIDATION?.PATTERNS?.TABLE_NUMBER || /^[1-9]\d*$/,
          phonePattern: window.APP_CONSTANTS?.VALIDATION?.PATTERNS?.PHONE || /^[\+]?[0-9\-\s\(\)]{8,}$/,
          minAddressLength: window.APP_CONSTANTS?.VALIDATION?.MIN_LENGTHS?.ADDRESS || 10
        },
        ui: {
          debounceDelay: window.APP_CONSTANTS?.UI_CONFIG?.DEBOUNCE_DELAY || 300,
          retryAttempts: window.APP_CONSTANTS?.VALIDATION?.MAX_RETRY_ATTEMPTS || 3
        }
      },
  ...
  computed: {
    ...
    discountAmount() {
      ...
      switch (this.activePromoCode.type) {
        ...
        case 'fixed':
          return Math.min(this.activePromoCode.value, subtotalAfterBulkDiscount);
        default:
          return 0;
      }
    },
    ...
    validationState() {
      return {
        tableNumber: this.serviceMethod === 'dine-in' ? this.validateTableNumber(this.tableNumber) : { isValid: true },
        phoneNumber: (this.serviceMethod === 'pickup' || this.serviceMethod === 'delivery') ? this.validatePhoneNumber(this.phoneNumber) : { isValid: true },
        deliveryAddress: this.serviceMethod === 'delivery' ? this.validateDeliveryAddress(this.deliveryAddress) : { isValid: true },
        promoCode: this.promoCode ? this.validatePromoCode(this.promoCode) : { isValid: true }
      };
    },
    
    formErrors() {
      const errors = [];
      if (this.serviceMethod === 'dine-in' && !this.validationState.tableNumber.isValid) {
        errors.push(this.validationState.tableNumber.message);
      }
      if ((this.serviceMethod === 'pickup' || this.serviceMethod === 'delivery') && !this.validationState.phoneNumber.isValid) {
        errors.push(this.validationState.phoneNumber.message);
      }
      if (this.serviceMethod === 'delivery' && !this.validationState.deliveryAddress.isValid) {
    ...
    deliveryFee() {
      ...
      return this.cartTotals.subtotal >= this.config.pricing.freeDeliveryThreshold ? 
             0 : this.config.pricing.deliveryFee;
    },
  ...
  methods: {
    ...
    validateTableNumber(value) {
      if (!value || !value.trim()) {
        return { isValid: false, message: 'Table number is required for dine-in orders' };
      }
    ...
    validateDeliveryAddress(value) {
      ...
      if (value.trim().length < this.config.validation.minAddressLength) {
        return { isValid: false, message: `Address must be at least ${this.config.validation.minAddressLength} characters` };
      }
      return { isValid: true, message: '' };
    },
    ...
    validatePromoCode(value) {
      ...
      if (promoCode.minOrder > 0 && this.cartTotals.subtotal < promoCode.minOrder) {
        return { isValid: false, message: `This code requires a minimum order of ${this.$currency(promoCode.minOrder)}` };
      }
      return { isValid: true, message: '' };
    },
    ...
    handleOffline() {
      this.showToast('warning', 'You are currently offline. Some features may not work properly.');
    ...
    async loadCart(retryCount = 0) {
      try {
        ...
        if (retryCount < this.config.ui.retryAttempts) {
          ...
          this.showToast('warning', `Loading cart... (attempt ${retryCount + 2})`);
        } else {
    ...
    recalculateCart() {
      try {
        ...
        const serviceCharge = discountedSubtotal * this.config.pricing.serviceChargeRate;
        const tax = (discountedSubtotal + serviceCharge) * this.config.pricing.taxRate;
    ...
    clearCart() {
      try {
        ...
        if (confirm(`Are you sure you want to clear your entire order? This will remove ${itemCount} items worth ${this.$currency(cartValue)}.`)) {
    ...
    applyPromoCode() {
      try {
        ...
        this.showToast('success', `Promo code applied! You saved ${this.$currency(this.promoDiscount)}`);
    ...
    async placeOrder() {
      try {
        ...
        if (!this.canPlaceOrder) {
          ...
          } else if (this.cartTotals.total < this.config.pricing.minOrderAmount) {
            message = `Minimum order amount is ${this.$currency(this.config.pricing.minOrderAmount)}`;
          }
        ...
        this.showToast('success', 'Order placed successfully! Your food will be prepared shortly.');
    ...
    async submitOrderWithRetry(orderData, attempt = 1) {
      try {
        ...
        if (attempt < this.config.ui.retryAttempts) {
          this.showToast('warning', `Retrying order submission... (attempt ${attempt + 1})`);
...
if (typeof window !== 'undefined') {
  ...
  try {
    ...
    if (typeof process !== 'undefined' && process.env && process.env.NODE_ENV === 'development') {
      console.log('✅ ShoppingCart Component Enhanced v2.0.0 - Registration Details:');
      console.log('   📦 Features: Cart management, service options, promotions, analytics');
      console.log('   🔧 Enhanced: Error handling, validation, retry logic, accessibility');
      ...
      console.log('   🚀 Performance: Optimized calculations, efficient state management');
    }
