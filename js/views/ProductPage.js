  ...
  data() {
    return {
      ...
      config: {
        ...
        loadingMessage:
          window.APP_CONSTANTS?.UI_LABELS?.LOADING_MENU ||
          "Loading menu items...",
        searchPlaceholder:
          window.APP_CONSTANTS?.UI_LABELS?.SEARCH_PLACEHOLDER ||
          "Search menu...",
        noResultsTitle:
          window.APP_CONSTANTS?.UI_LABELS?.NO_RESULTS || "No menu items found",
        noResultsMessage:
          window.APP_CONSTANTS?.UI_LABELS?.NO_RESULTS_MESSAGE ||
        ...
        searchMinLength:
          window.APP_CONSTANTS?.VALIDATION?.MIN_SEARCH_LENGTH || 2,
        searchMaxLength:
          window.APP_CONSTANTS?.VALIDATION?.MAX_SEARCH_LENGTH || 50,
        ...
        scrollBehavior:
          window.APP_CONSTANTS?.UI_CONFIG?.SCROLL_BEHAVIOR || "smooth",
    ...
    filteredProducts() {
      ...
      const filtered = this.products
        .filter((product) => {
          ...
          const matchesSearch =
            !this.searchQuery || this.matchesSearchQuery(product);

          // Price range filter
          const effectivePrice = this.getEffectivePrice(product);
          const matchesPrice =
            effectivePrice >= this.priceRange.min &&
            effectivePrice <= this.priceRange.max;
          ...
          const matchesDiscount =
            !this.onlyDiscount || (product.discount && product.discount > 0);

          // Stock filter
          const matchesStock =
            !this.onlyInStock || (product.stock && product.stock > 0);
          ...
          const matchesPrepTime =
            ...
            product.preparationTime <= this.maxPrepTime;

          // Calories filter
          const matchesCalories =
            !product.calories || product.calories <= this.maxCalories;
      ...
      if (this.config.enableAnalytics && filterTime > 50) {
        console.log(
          `Filter operation took ${filterTime.toFixed(2)}ms for ${
    ...
    displayedPageNumbers() {
      ...
      if (total <= maxPagesToShow) {
        ...
        if (current <= 3) {
          ...
          for (
            let i = Math.max(total - maxPagesToShow + 2, 2);
            i <= total;
            i++
          ) {
            pages.push(i);
    ...
    activeFilterSummary() {
      ...
      if (this.selectedCategory) {
        filters.push({
          type: "category",
          label: "Category",
          value: this.getCategoryLabel(this.selectedCategory),
        });
      }
      if (this.searchQuery) {
        filters.push({
      ...
      if (this.sortOption !== "default") {
        filters.push({
          type: "sort",
          label: "Sort",
          value: this.getSortLabel(this.sortOption),
        });
      }
      if (this.minRating > 0) {
        filters.push({
      ...
      if (this.selectedDietaryOptions.length > 0) {
        const options = this.selectedDietaryOptions
          .map((id) => {
    ...
    productStats() {
      ...
      try {
        ...
        return {
          ...
          discountedItems: products.filter((p) => p.discount && p.discount > 0)
            .length,
      ...
      handler(newProduct, oldProduct) {
        if (newProduct) {
          this.specialInstructions =
            newProduct.customization?.specialInstructions || "";
          this.quantity = 1;
  ...
  methods: {
    ...
    handleLoadError(message, error) {
      ...
      if (this.retryCount < this.config.maxRetries) {
        this.loadError = `${message}. Retrying... (${this.retryCount + 1}/${
    ...
    async loadProductDetails(productId) {
      try {
        this.currentProduct = await window.ProductService.getProductById(
          productId
        );
    ...
    async loadRelatedProducts() {
      try {
        ...
        const categoryProducts =
          await window.ProductService.getProductsByCategory(
    ...
    getCategoryLabel(categoryId) {
      ...
      return category
        ? category.name
        : window.ProductService.getCategoryIcon(categoryId);
    },
    ...
    isRemovedIngredient(ingredient) {
      ...
      return this.currentProduct.customization.removedIngredients.includes(
        name
      );
    },
    ...
    showAddToCartSuccess() {
      ...
      const customizationText = hasCustomization
        ? this.config.customizedItemSuffix
        : "";
    ...
    validatePriceRange(type, value) {
      ...
      if (type === "min") {
        if (numValue < 0) {
          this.validation.errors.priceRange =
        ...
        if (numValue > this.config.maxPriceRange) {
          this.validation.errors.priceRange = `Maximum price cannot exceed ${this.formatCurrency(
            this.config.maxPriceRange
          )}`;
          return false;
        }
      }
    ...
    handleProductUpdated(event) {
      if (
        ...
        this.currentProduct?.id === event.detail.productId
      ) {
    ...
    updateUrlParams() {
      ...
      const hasChanges =
        Object.keys(params).some((key) => params[key] !== currentQuery[key]) ||
...
(function () {
  ...
  try {
    ...
    window.ProductPage._metadata = {
      ...
      dependencies: {
        required: ["Vue.js", "ProductService", "APP_CONSTANTS"],
        optional: [
          "CartService",
          "ToastService",
          "ErrorHandler",
    ...
    window.ProductPage.healthCheck = function () {
      ...
      try {
        ...
        health.dependencies.Vue =
          typeof Vue !== "undefined" ? "available" : "missing";
        health.dependencies.ProductService =
          typeof window.ProductService !== "undefined"
            ? "available"
            : "missing";
        health.dependencies.APP_CONSTANTS =
          typeof window.APP_CONSTANTS !== "undefined" ? "available" : "missing";

        // Check optional dependencies
        health.dependencies.CartService =
          typeof window.CartService !== "undefined" ? "available" : "optional";
        health.dependencies.ToastService =
          typeof window.ToastService !== "undefined" ? "available" : "optional";
        health.dependencies.ErrorHandler =
          typeof window.ErrorHandler !== "undefined" ? "available" : "optional";
        health.dependencies.ValidationService =
          typeof window.ValidationService !== "undefined"
            ? "available"
            : "optional";
        health.dependencies.Analytics =
          typeof window.analytics !== "undefined" ? "available" : "optional";
        health.dependencies.Helpers =
          typeof window.Helpers !== "undefined" ? "available" : "optional";
        ...
        health.capabilities.sessionStorage =
        ...
        const criticalMissing = Object.entries(health.dependencies)
          .filter(
            ([key, status]) =>
              status === "missing" &&
              ["Vue", "ProductService", "APP_CONSTANTS"].includes(key)
          )
          .map(([key]) => key);
        ...
        if (criticalMissing.length > 0) {
          ...
          if (optionalMissing.length > 2) {
            ...
            health.issues.push(
              `Multiple optional services unavailable: ${optionalMissing.join(
                ", "
              )}`
            );
          } else {
    ...
    window.ProductPage.debug = {
      ...
      testInstantiation() {
        try {
          ...
          return {
            success: true,
            message: "Component instantiation successful",
          };
        } catch (error) {
          return {
            success: false,
    ...
    if (window.APP_CONSTANTS?.DEBUG?.COMPONENTS) {
      console.log(
        "ProductPage Debug Info:",
        window.ProductPage.debug.getInfo()
      );
    }
  } catch (error) {
