...
class ValidationService {
  constructor() {
    this.config = {
      ...
      MAX_PASSWORD_LENGTH:
        window.APP_CONSTANTS?.VALIDATION?.MAX_PASSWORD_LENGTH || 128,
      ...
      MIN_PHONE_LENGTH:
        window.APP_CONSTANTS?.VALIDATION?.MIN_PHONE_LENGTH || 10,
      MAX_PHONE_LENGTH:
        window.APP_CONSTANTS?.VALIDATION?.MAX_PHONE_LENGTH || 15,
    };
  ...
  validateRequired(value, fieldName) {
    ...
    return {
      ...
      errors: isValid
        ? []
        : [this.messages.REQUIRED.replace("This field", fieldName)],
    };
  }
  ...
  validatePhone(value, fieldName) {
    ...
    const isValid =
      this.patterns.PHONE.test(cleanPhone) &&
  ...
  validateCreditCard(value, fieldName) {
    ...
    const isValid =
      this.patterns.CREDIT_CARD.test(cleanValue) && this.luhnCheck(cleanValue);
  ...
  setFieldValidation(fieldName, validationResult) {
    try {
      ...
      if (!fieldElement) {
        console.warn(
          `ValidationService: Field element not found for ${fieldName}`
        );
        return;
      }
      ...
      const existingError = fieldElement.parentNode.querySelector(
      ...
      if (validationResult.isValid) {
        ...
        if (validationResult.errors && validationResult.errors.length > 0) {
          ...
          errorDiv.className =
            "validation-message error-message text-danger small mt-1";
      ...
      if (validationResult.warnings && validationResult.warnings.length > 0) {
        ...
        warningDiv.className =
          "validation-message warning-message text-warning small mt-1";
