...
class ProductService {
  ...
  async init() {
    ...
    try {
      ...
      window.ErrorHandler?.logError(error, { service: 'ProductService', method: 'init' });
  ...
  async loadProductsWithRetry() {
    ...
    throw new Error(`Failed to load products after ${this.config.MAX_RETRIES} attempts: ${lastError.message}`);
  }
  ...
  filterProductsAdvanced(criteria = {}) {
    ...
    if (criteria.category && criteria.category !== '') {
      filtered = filtered.filter(product => product.category === criteria.category);
    }
    ...
    if (criteria.minRating && criteria.minRating > 0) {
      filtered = filtered.filter(product => product.rating >= criteria.minRating);
    }
    ...
    if (criteria.maxPrepTime) {
      filtered = filtered.filter(product => 
        product.preparationTime <= criteria.maxPrepTime
      );
    }
  ...
  searchProducts(query, options = {}) {
    ...
    this.menuItems.forEach(product => {
      ...
      const searchableContent = {
        ...
        ingredients: (product.ingredients || []).map(ing => ing.name.toLowerCase()).join(' '),
