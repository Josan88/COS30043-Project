...
window.app.directive('focus-trap', {
  mounted(el, binding) {
    ...
    el._focusTrapHandler = function(e) {
      ...
      // Close on Escape key
      if (e.key === 'Escape' && binding.value && typeof binding.value === 'function') {
        binding.value();
      }
    };
  ...
  mounted(el, binding) {
    ...
    el._validateHandler = function() {
      ...
      if (config.required && !value) {
        isValid = false;
        errorMessage = config.requiredMessage || `${config.label || 'This field'} is required`;
      }
      
      // Pattern validation
      if (isValid && config.pattern && !new RegExp(config.pattern).test(value)) {
        isValid = false;
        errorMessage = config.patternMessage || 'Invalid format';
      }
      
      // Custom validation function
      if (isValid && config.validator && typeof config.validator === 'function') {
      ...
      headers.forEach(th => {
        ...
        th._sortHandler = function(e) {
          if (e.type === 'click' || (e.type === 'keydown' && (e.key === 'Enter' || e.key === ' '))) {
            e.preventDefault();
            if (binding.value.onSort && typeof binding.value.onSort === 'function') {
  ...
  mounted(el, binding) {
    ...
    el._errorHandler = function() {
      ...
      if (config.fallback) {
        ...
        textAlt.className = 'image-placeholder text-center p-3 bg-light border rounded';
...
window.app.directive('scroll-reveal', {
  mounted(el, binding) {
    ...
    el.style.transition = `opacity ${config.duration || '0.6s'} ease, transform ${config.duration || '0.6s'} ease`;
